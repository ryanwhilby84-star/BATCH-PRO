// Batch ID Pro — Single-File App (FIXED BUILD)
// QR codes point to resolver at https://batch.coresystemsni.com/?receipt=ID&d=ENCODED
// No router. Vite + React. TypeScript-safe. No duplicate helpers.
...
export default function App() {
    ...
    if (receiptId) {
      if (encoded) {
        try {
          ...
          console.warn("Receipt decode failed", e);
        }
      }
      ...
      return (
        <div style={{ minHeight: "100vh", display: "grid", placeItems: "center", background: "#0b1220", padding: 24 }}>
          ...
          >
            <div style={{ fontSize: 18, fontWeight: 900, color: "white", marginBottom: 8 }}>Receipt link incomplete</div>
            <div style={{ color: "rgba(255,255,255,0.75)", fontSize: 13, lineHeight: 1.5 }}>
              This link has a receipt code (<b style={{ color: "white" }}>{receiptId}</b>) but is missing the encoded receipt data.
              <br />
              Open the batch in the app and use the newly generated QR code.
            </div>
            ...
            <div style={{ marginTop: 10, color: "rgba(255,255,255,0.45)", fontSize: 11, wordBreak: "break-all" as const }}>
  ...
  const archiveBatch = useCallback(
    (id: string) => {
      updateBatch(id, { archived: true, status: "Archived" });
      addToast("success", "Archived");
    },
    [updateBatch, addToast]
  );

  const unarchiveBatch = useCallback(
    (id: string) => {
      updateBatch(id, { archived: false, status: "Created" });
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
        {(
          [
            ...
            ["outbound", "📤", "Create Outbound", "New outgoing docket"],
            ["batches", "📋", "All Batches", "View batch records"],
...
function BatchesView({ batches, tab, setTab, openBatch, deleteBatch }: any) {
  return (
    <div>
      ...
      {batches.length === 0 ? (
        <div style={{ ...S.card, padding: 24, textAlign: "center" as const, opacity: 0.5 }}>No batches in this status</div>
      ) : (
...
function CreateDocketView({ batchType, createBatch, speciesLibrary, companyLibrary, addToast, setView }: any) {
  ...
  const updateSpecies = (i: number, f: keyof SpeciesLine, v: any) => setSpeciesLines((p: SpeciesLine[]) => p.map((l, idx) => (idx === i ? { ...l, [f]: v } : l)));
  ...
  const handleCreate = () => {
    ...
    const batch: Batch = {
      ...
      speciesLines: speciesLines
        .map((l) => ({ ...l, weightKg: Number(l.weightKg) }))
        .filter((l) => l.species),
      transportLegs: [],
      landingPort,
      processingPlant,
      ...
      <div style={S.cardPad}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap" as const, gap: 10, marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" as const }}>
            {batch.status === "Created" && batch.batchType === "outbound" && (
              <button
                style={{ ...S.btn, ...S.btnPrimary }}
                onClick={() => {
                  updateBatch(batch.id, { status: "In Transit" });
                  addToast("success", "Marked In Transit");
                }}
              >
                Mark In Transit
              </button>
            )}
            {(batch.status === "Created" || batch.status === "In Transit") && (
              <button
                style={{ ...S.btn, ...S.btnPrimary }}
                onClick={() => {
                  updateBatch(batch.id, { status: "Completed" });
                  addToast("success", "Completed");
                  closeBatch();
                }}
              >
                Mark Complete
        ...
        <div style={{ marginBottom: 16 }}>
          <h3 style={{ fontSize: 13, fontWeight: 800, marginBottom: 10, opacity: 0.7, textTransform: "uppercase" as const, letterSpacing: 1 }}>
            Species & Weight
          </h3>
        ...
        <h3 style={{ fontSize: 13, fontWeight: 800, marginBottom: 12, opacity: 0.7, textTransform: "uppercase" as const, letterSpacing: 1 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <h3 style={{ fontSize: 13, fontWeight: 800, opacity: 0.7, textTransform: "uppercase" as const, letterSpacing: 1 }}>
            Transport / Handover Logs
          </h3>
          <button style={{ ...S.btn, fontSize: 11, padding: "6px 10px" }} onClick={addLeg}>
            + Add Leg
          </button>
        </div>
        ...
        {transportLegs.map((leg, i) => (
          <div key={i} style={{ ...S.subCard, marginBottom: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
              ...
              <button style={{ ...S.btn, ...S.btnDanger, fontSize: 11, padding: "3px 8px" }} onClick={() => removeLeg(i)}>
                Remove
              </button>
            </div>
            ...
            <div style={S.formGrid}>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateLeg(i, "transportCompany", e.target.value)}
                />
              ...
              <div>
                ...
                <input style={S.input} value={leg.vehicleReg} onChange={(e) => updateLeg(i, "vehicleReg", e.target.value)} />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateLeg(i, "handoverTime", e.target.value)}
                />
              </div>

              <div>
                <label style={S.label}>Notes</label>
                <input style={S.input} value={leg.notes || ""} onChange={(e) => updateLeg(i, "notes", e.target.value)} />
              </div>
            </div>
          </div>
        ))}

        <button style={{ ...S.btn, ...S.btnPrimary, width: "100%", marginBottom: 16 }} onClick={saveCerts}>
        ...
        {batch.notes && (
          <div style={{ marginBottom: 16 }}>
            <h3 style={{ fontSize: 13, fontWeight: 800, marginBottom: 8, opacity: 0.7, textTransform: "uppercase" as const, letterSpacing: 1 }}>Notes</h3>
            <p style={{ fontSize: 13, lineHeight: 1.6, opacity: 0.8 }}>{batch.notes}</p>
          </div>
        )}
...
function EOWView({ batches, openBatch }: any) {
  ...
  return (
    <div>
      ...
      <p style={{ ...S.small, opacity: 0.5, marginBottom: 20 }}>
        {batches.length} completed batches · {totalWeight} kg total
      </p>
      {batches.length === 0 ? (
        <div style={{ ...S.card, padding: 24, textAlign: "center" as const, opacity: 0.5 }}>No completed batches yet</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column" as const, gap: 10 }}>
          {batches.map((b: Batch) => (
...
function ArchiveView({ batches, openBatch, unarchiveBatch }: any) {
  return (
    <div>
      ...
      {batches.length === 0 ? (
        <div style={{ ...S.card, padding: 24, textAlign: "center" as const, opacity: 0.5 }}>No archived batches</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column" as const, gap: 10 }}>
          {batches.map((b: Batch) => (
            <div
    ...
    <div style={S.card}>
      <div style={S.cardPad}>
        ...
        <div style={{ marginBottom: 16, padding: 12, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, fontSize: 13 }}>
          ...
          <br />
          <span style={{ fontFamily: "monospace", fontSize: 11, opacity: 0.6 }}>{RESOLVER_BASE}/?receipt=ID&d=ENCODED</span>
          <p style={{ marginTop: 6, opacity: 0.5, fontSize: 12 }}>QR codes point here. Your resolver domain must be live.</p>
        </div>
